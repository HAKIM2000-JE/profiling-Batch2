...
def run():
    ...
    for index, Region in Regions.iterrows():
        ...
        if not Properties.empty:
            ...
            ListPropertyId = [row['_id']
                              for index, row in Properties.iterrows()]

            Database.DATABASE[environement.COLLECTION_REGION].update_one(
                {"_id": Region['_id']}, {"$unset": {'propertiesIds': 1}})

            # Mettre a jour la list des propertiesIds par region
            Database.DATABASE[environement.COLLECTION_REGION].update_one(
                {"_id": Region['_id']}, {"$set":  {'propertiesIds': ListPropertyId}})
            ...
            if not Reservations.empty:
                ...
                for Profile in Profiles:
                    ...
                    Profile_Object['propertyBookingsIds'] = Profile.get(
                        'propertyBookingsIds')
                    ...
                    if not Recommendations.empty:
                        ...
                        if len(Recommendation_Output) > 0:
                            ...
                            for recommendation in Recommendation_Output:
                                ...
                                RecommendationScore = {}
                                RecommendationScore['recommendationId'] = recommendation.get(
                                    'Recommendation Id')
                                RecommendationScore['poi'] = recommendation.get(
                                    'poi')
                                RecommendationScore['score'] = recommendation.get(
                                    'score')
            ...
            if len(FinalProfilesList) > 0:
                ...
                Database.DATABASE[environement.COLLECTION_PROFILE].delete_many(
                    {"regionId": Region['_id']})
                # Ajout des nouveau profile de la région
                Database.DATABASE[environement.COLLECTION_PROFILE].insert_many(
                    FinalProfilesList)
